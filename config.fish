if status is-interactive
    ...
    ### SIGMA ###
    abbr --add sshs 'ssh oskarei@saga.sigma2.no'
    abbr --add sshf 'ssh fox -M'
    ...
    ### DISPLAY MANAGER
    set -l DA hyprlock
    abbr --add reload_da "hyprctl --instance 0 'keyword misc:allow_session_lock_restore 1' ; hyprctl --instance 0 'dispatch exec $DA'"
...
#   abbr --add cndd 'conda deactivate ; conda deactivate'
  ...
  # abbr --add gita  --set-cursor 'git add -A && git commit -m "%"'
    ...
    abbr --add cdt 'cd ~/repos/MasterThesis/tex'
    abbr --add cdm 'cd ~/repos/TargetedAlphaTherapy'
    abbr --add cdp 'cd ~/repos/PhD/'
    abbr --add osk --position anywhere /mnt/C/Users/Oskar
    abbr --add ':null' --position anywhere '> /dev/null'
    ...
    # --- Envirnoment ---
    # ROOT
    source ~/root_src_v.6.36.02/BUILD/bin/thisroot.fish

     ~/geant4-v11.3.2-installQt/bin/geant4.sh
...
#    source /usr/share/LS_COLORS/dircolors.csh
    ...
    ### NMTUI ###
    set -x NEWT_COLORS 'root=black;window=black,black;border=white,black;label=blue,black;checkbox=red,black;title=green,black;button=white,red;actsellistbox=white,red;actcheckbox=white,blue;entry=lightgray,black;textbox=blue,black' nmtui

    
    ### OTHER ###
    alias refresh='reload-fish ; clear ; fish_greeting'
...
set -x PATH (python -c "import os; print(':'.join(dict.fromkeys(map(os.path.normpath, os.environ['PATH'].split(':'))).keys()))")
